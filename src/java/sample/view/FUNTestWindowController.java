...
public class FUNTestWindowController {
    ...
    private void start() {
        ...
        Task<Void> testTask = new Task<>() {
            ...
            protected Void call() throws Exception {
                try {
                    if (!main.getCurrentSettings().get(TestSetting.RECHECK)) {
                        writeFUNPower();
                    }
                    runFans();
    ...
    private void writeFUNPower() throws ParserConfigurationException, TransformerException {
        ConnectHandler.CUset("SlotPowerSet", String.valueOf(main.getFun().getPower()));
        ConnectHandler.CUset("SlotNameSet", main.getCurrentSNs().get(Part.FUNPN));
    ...
    private void writeFUNInfo() throws ParserConfigurationException, TransformerException {
    ...
    private boolean checkFUNInfo() throws ParserConfigurationException, IOException, SAXException, TransformerException {
        if (main.getCurrentSettings().get(TestSetting.RECHECK)) {
            results.put(FUNTestResult.FUN_NAME, !ConnectHandler.CUget("FUName").isEmpty());
        } else {
            results.put(FUNTestResult.FUN_NAME, ConnectHandler.CUget("FUName").equals(main.getCurrentSNs().get(Part.FUNPN)));
        }
    ...
    private boolean testFUN10() throws InterruptedException, ParserConfigurationException, TransformerException, SAXException, IOException {
        return testFUNSpeed(FUNTestResult._10, 10);
    }
    ...
    private boolean testFUN100() throws InterruptedException, ParserConfigurationException, TransformerException, SAXException, IOException {
        return testFUNSpeed(FUNTestResult._100, 100);
    }

    private boolean testFUNSpeed(FUNTestResult funTestResult, int speed) throws ParserConfigurationException, IOException, SAXException, TransformerException, InterruptedException {
        if (results.containsKey(funTestResult) && results.get(funTestResult)) {
            return true;
        }
        ...
        for (int i = 0; i < 23; i++) {
            ...
            if (fanSpeeds().values().stream().noneMatch(p -> (p < finalLow || p > finalHigh))) {
                results.put(funTestResult, true);
                return true;
            }
        }
        results.put(funTestResult, false);
        return false;
    }

    private void setFUNAuto() throws ParserConfigurationException, TransformerException {
